...
samples=${out_dir}/dev_pos_embeddings_beam=${beam_size}.samples
ptb_samples=${out_dir}/dev_pos_embeddings_beam=${beam_size}.ptb_samples
...
# build/nt-parser/nt-parser --cnn-mem 7000,0,800 -x -T corpora/train.oracle -p corpora/dev.oracle -C corpora/dev.stripped --pretrained_dim 100 -w embeddings/sskip.100.filtered.vectors -P --lstm_input_dim 128 --hidden_dim 128 -m $discrim_model --alpha 0.8 --beam_size $beam_size --output_beam_as_samples --ptb_output_file $ptb_samples > $samples 2> ${samples}.stderr
