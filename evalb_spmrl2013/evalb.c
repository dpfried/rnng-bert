...
/*                                         October.1997          			*/
/*                                                               			*/
/*			  -K n		Evaluate up to  n sentences							*/
/*            -X		Count skipped sentences brackets as not parsed		*/
/*            -L		Compact view (for use in batch mode, all sentences	*/
/*			  -V		Display test and gold files							*/
/*			  -F		Evaluate with function labels						*/
/*										Djamé Seddah (Univ. Paris Sorbonne) */
/*										2013-2014							*/
/****************************************************************************/
...
// f- ormer version was 78x slower than regular evalb.
// - fixing evaluation on bad punct pos

// DJamé post shared task stuff
// - adding new option to evaluate the labels
...
int spmrl_keep_function_labels=0; //default not to evaluate function
char spmrl_func_marker='-'; // default is to use this to separate the function
/************************/
			...
			while(*++(*argv)){
				switch(**argv){
					...
					case 'V': // added by djame to add gold_name vs test_file in the outpu
						...
						goto nextarg;
					case 'F': // to keep the function label
						spmrl_keep_function_labels=1;
						spmrl_func_marker='\0'; // Djame: then func labeles are evaluated ( '-' otherwise)
						goto nextarg;
					default:
						Usage();
						exit(0);
...
char *label;
{
    ...
    for(p=label;*p!='\0';p++){
        //djame: was *p=='-'
		if(*p==spmrl_func_marker || *p=='='|| *p=='#'){ // for dealing with  morph features
			*p='\0';
			break;
...
Usage()
{
    ...
    fprintf(stderr,"    -V			   print gold and test files   \n");
    fprintf(stderr,"    -F             Keeps function labels for evaluation\n");
