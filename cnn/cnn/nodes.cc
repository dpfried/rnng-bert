...
#ifdef HAVE_CUDA
  if (pval) {
    gpu::pick(xs[0]->d.size(), *pval, xs[0]->v, fx.v);
  } else {
    throw std::runtime_error("batched PickElement not yet implemented for CUDA");
  }
#else
  if(pval) {
    if (*pval >= xs[0]->d.rows()) {
...
#ifdef HAVE_CUDA
  if (pval) {
    gpu::pick_backward(xs[0]->d.size(), *pval, dEdf.v, dEdxi.v);
  } else {
    throw std::runtime_error("batched PickElement not yet implemented for CUDA");
  }
#else
