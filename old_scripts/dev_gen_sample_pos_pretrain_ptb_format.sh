...
samples=${out_dir}/dev_pos_embeddings_s=${num_samples}.ptb_samples
...
build/nt-parser/nt-parser --cnn-mem 1700 -x -T corpora/train.oracle -p corpora/dev.oracle -C corpora/dev.stripped --pretrained_dim 100 -w embeddings/sskip.100.filtered.vectors -P --lstm_input_dim 128 --hidden_dim 128 -m $discrim_model --alpha 0.8 -s $num_samples --ptb_output_format > $samples 2> ${samples}.stderr
