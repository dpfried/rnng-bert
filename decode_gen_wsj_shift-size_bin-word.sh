...
if [ "$#" -ne 6 ]; then
    echo "args: beam_size beam_size_at_word shift_size word_sig_length word_sig_type block_num"
    exit 1
fi
...
prefix="rnng_gen-beam_size=${beam_size}-at_word=${at_word}-shift_size=${shift_size}-word_sig_length=${word_sig_length}-word_sig_type=${word_sig_type}"
...
gdb -args build/nt-parser/nt-parser-gen \
    --cnn-mem 4000,0,500 \
    ...
    --decode_word_sig_length $word_sig_length \
    --decode_word_sig_type $word_sig_type \
