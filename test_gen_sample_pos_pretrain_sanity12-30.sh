...
in_dir="../rnng_adhi/expts_sampling"
out_dir="expts_sampling_sanity12-30/"
...
fname="test_pos_embeddings_s=${num_samples}.samples"
samples=${out_dir}/$fname
cp ${in_dir}/${fname} $samples
...
# build/nt-parser/nt-parser --cnn-mem 1700 -x -T corpora/train.oracle -p corpora/test.oracle -C corpora/test.stripped --pretrained_dim 100 -w embeddings/sskip.100.filtered.vectors -P --lstm_input_dim 128 --hidden_dim 128 -m $discrim_model --alpha 0.8 -s $num_samples > $samples 2> ${samples}.stderr
